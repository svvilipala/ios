...
class Game{
    ...
    init(name: String?, information: String?, imageName: UIImage?) {
        self.name = name
...
class VilipalaHomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return gameArray.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = VilipalaTVOL.dequeueReusableCell(withIdentifier: "VilipalaCell", for: indexPath)
    ...
    @IBOutlet weak var VilipalaTVOL: UITableView!
    var gameArray = [Game]()
    ...
    override func viewDidLoad() {
        ...
        VilipalaTVOL.dataSource = self
        VilipalaTVOL.delegate = self
        ...
        gameArray.append(trackandField)
        ...
        gameArray.sort { ($0.name ?? "") < ($1.name ?? "") }
            VilipalaTVOL.reloadData()

    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let transition = segue.identifier
        if transition == "VilipalaDescriptionSegue" {
            let destination = segue.destination as! VilipalaGameController
            destination.SelectedGame = gameArray[(VilipalaTVOL.indexPathForSelectedRow?.row)!]
        }
